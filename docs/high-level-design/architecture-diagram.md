...
---
## Implementation-Ready Architecture Diagram
...
### Mermaid architecture (service boundaries)
```mermaid
flowchart LR
  Client --> API
  API --> SlotSvc
  ...
  SlotSvc --> Cache[(Redis)]
  BookingSvc --> Bus[(Event Bus)]
  PaymentSvc --> PSP[(Gateway)]
  Bus --> NotifySvc
  BookingSvc --> Obs[(Telemetry Stack)]
```
