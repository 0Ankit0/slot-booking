...
---
## Implementation-Ready Data Flow Diagram
...
### Mermaid data flow for confirm/cancel/refund
```mermaid
flowchart TD
  Req[Client request] --> Avail[Availability read]
  Avail --> Hold[Hold write]
  Hold --> Pay[Payment auth/capture]
  Pay --> Book[Booking state update]
  Book --> Ledger[Financial ledger]
  Cancel[Cancellation request] --> Policy[Refund policy engine]
  Policy --> Refund[Refund transaction]
  Refund --> Ledger
```
