...
---
## Implementation-Ready Bpmn Swimlane Diagram
...
### Mermaid swimlane-style process
```mermaid
flowchart LR
  subgraph Customer
    C1[Select slot] --> C2[Submit booking]
  end
  subgraph BookingService
    B1[Reserve hold] --> B2[Validate conflict]
    B2 --> B3[Confirm booking]
    B4[Cancel and release]
  end
  subgraph PaymentService
    P1[Authorize/Capture] --> P2[Callback]
    P3[Refund]
  end
  C2 --> B1
  B2 --> P1
