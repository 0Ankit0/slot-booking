...
---
## Implementation-Ready Activity Diagram
...
### Mermaid activity with allocation and payment branches
```mermaid
flowchart TD
  A[Check slot] --> B[Create hold]
  B --> C{Policy mode}
  C -->|Coupled| D[Authorize payment]
  C -->|Decoupled| E[Create pending booking]
  D --> F{Authorized?}
  F -->|Yes| G[Confirm booking]
  F -->|No| H[Release hold]
  E --> I{Paid within grace?}
  I -->|Yes| J[Mark paid]
  I -->|No| K[Auto-cancel]
```
