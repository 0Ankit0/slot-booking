...
---
## Implementation-Ready Class Diagram
...
### Mermaid class model for booking/payment/refund policies
```mermaid
classDiagram
  class Slot {id resourceId start end capacity version}
  class Hold {id slotId userId expiresAt status}
  class Booking {id slotId status paymentMode}
  class PaymentTxn {id bookingId amount status}
  class RefundCase {id bookingId decision status}
  Slot "1" --> "many" Hold
  Slot "1" --> "many" Booking
  Booking "1" --> "many" PaymentTxn
  Booking "1" --> "many" RefundCase
```
